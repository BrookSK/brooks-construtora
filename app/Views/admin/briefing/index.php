...
<button type="button" class="btn btn-outline-secondary flex-grow-1" onclick="openContractorModal(0)"><i class="bi bi-plus-lg me-1"></i> Nova</button>
<button type="button" class="btn btn-outline-primary" id="btn-edit-contractor" title="Editar empresa" onclick="editContractor()" style="display:none;"><i class="bi bi-pencil"></i></button>
<button type="button" class="btn btn-outline-danger" id="btn-del-contractor" title="Excluir empresa" onclick="deleteContractor()" style="display:none;"><i class="bi bi-trash"></i></button>
</div>
...
function openContractorModal(id){
    document.getElementById('ct-id').value=id||0;
    document.getElementById('ct-modal-title').textContent=(id?'Editar':'Nova')+' Empresa Contratada';
    if(!id){['ct-company_name','ct-trade_name','ct-cnpj','ct-address','ct-address_number','ct-complement','ct-neighborhood','ct-city','ct-state','ct-cep','ct-phone','ct-email','ct-representative_name','ct-representative_role'].forEach(function(f){ctSet(f,'');});}
    var fb=document.getElementById('ct-feedback');if(fb)fb.textContent='';
    new bootstrap.Modal(document.getElementById('modalContractor')).show();
}
function editContractor(){
    var sel=document.getElementById('contractor_company_id');if(!sel||!sel.value){return;}
    var opt=sel.querySelector('option[value="'+sel.value+'"]');var c={};try{c=JSON.parse(opt.dataset.json||'{}');}catch(e){}
    openContractorModal(sel.value);
    ctSet('ct-company_name',c.company_name);ctSet('ct-trade_name',c.trade_name);ctSet('ct-cnpj',c.cnpj);
    ctSet('ct-address',c.address);ctSet('ct-address_number',c.address_number);ctSet('ct-complement',c.complement);
    ctSet('ct-neighborhood',c.neighborhood);ctSet('ct-city',c.city);ctSet('ct-state',c.state);ctSet('ct-cep',c.cep);
    ctSet('ct-phone',c.phone);ctSet('ct-email',c.email);ctSet('ct-representative_name',c.representative_name);ctSet('ct-representative_role',c.representative_role);
}
function deleteContractor(){
    var sel=document.getElementById('contractor_company_id');if(!sel||!sel.value){return;}
    var nm=sel.options[sel.selectedIndex].text;
    if(!confirm('Excluir a empresa "'+nm+'"?\nEsta ação remove apenas o cadastro da empresa.'))return;
    var fd=new FormData();fd.append('id',sel.value);
    fetch('/admin/briefing/delete-contractor',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
        if(d.success){var o=sel.querySelector('option[value="'+sel.value+'"]');if(o)o.remove();sel.value='';loadContractor('');showToast('Empresa excluída.','success');}
        else{alert(d.error||'Erro ao excluir.');}
    }).catch(()=>alert('Falha ao excluir.'));
}
function submitContractor(){
    var fb=document.getElementById('ct-feedback'),id=document.getElementById('ct-id').value,name=document.getElementById('ct-company_name').value.trim();
    if(!name){fb.textContent='Razão social obrigatória.';fb.className='small text-danger';return;}
    var fd=new FormData();
    if(id&&id!=='0')fd.append('id',id);
    ['company_name','trade_name'].forEach(f=>fd.append(f,document.getElementById('ct-'+f).value.trim()));
    fd.append('contractor_cnpj',document.getElementById('ct-cnpj').value.trim());
    ['address','address_number','complement','neighborhood','city','state','cep','phone','email','representative_name','representative_role'].forEach(f=>fd.append('contractor_'+f,document.getElementById('ct-'+f).value.trim()));
    var url=(id&&id!=='0')?'/admin/briefing/update-contractor':'/admin/briefing/store-contractor';
    fetch(url,{method:'POST',body:fd}).then(r=>r.json()).then(data=>{
        if(data.success){
            fb.textContent='Salvo!';fb.className='small text-success';
            var sel=document.getElementById('contractor_company_id');
            ...
            opt.textContent=data.company.company_name+(data.company.cnpj?' — '+data.company.cnpj:'');
            opt.dataset.json=JSON.stringify(data.company);
            sel.value=data.id;loadContractor(data.id);
            setTimeout(()=>{bootstrap.Modal.getInstance(document.getElementById('modalContractor')).hide();fb.textContent='';},700);
        }else{fb.textContent=data.error||'Erro.';fb.className='small text-danger';}
    }).catch(()=>{fb.textContent='Erro.';fb.className='small text-danger';});
}
...
/* Máscara monetária pt-BR: exibe 000.000,00 conforme digita, mantém centavos */
(function(){
    function maskMoney(el){
        ...
        var cents=digits.slice(-2),intp=digits.slice(0,-2);
